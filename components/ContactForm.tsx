...
export default function ContactForm({ defaultService }: ContactFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const subject = 'New Lead: ' + (form.service || 'Service Inquiry') + ' - ' + form.name
      const data = {
        name: form.name,
        phone: form.phone,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <select
          ...
          value={form.service}
          onChange={handleChange}
          className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
        >
